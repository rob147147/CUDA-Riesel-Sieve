...
//Seem to not be printing factors from the final kernel - think this is now fixed
...
void generateGPUPrimes(unsigned int *KernelP1, unsigned long long low, unsigned int *smallP, int testArraySize, int primeCount, int arraySize, unsigned long long *mark1, unsigned long long **maskNew, int numOfMasks);
...
void countKs(char* abcdFile, int& count1, int& count3) {
	...
	unsigned long long* allocate_and_initialize(size_t elements) {
    		unsigned long long *mask = (unsigned long long *)malloc(elements * sizeof(unsigned long long));
    		if (mask != NULL) {
        		memset(mask, 0, elements * sizeof(unsigned long long));
    		}
    		return mask;
	}
				...
				for (int t = tMin[0]; t < tMax[0]; t++) {
					...
					while (true) {
						...
						if (sbHigh == key) {
							...
							//This prints for every false positive. So we had (sbHigh == key) but no hit in the hash table.
							//printf("Match in S %d. t=%d, hash=%d, probe=%d, beta=%llu, rem=%d, pointer=%d. Output will be %llu | %d*%d^%d-1\n", S, t, hash, probe, beta, remainder, sbInt - sbHigh, b, thisk, base[0], ((output* *Q) + remainder));
...
int main(int argc, char* argv[])
{
 ...
 char *FactorFile = "factors.txt";
	...
	while ((inp = getopt(argc, argv, "b:h:i:m:p:P:Q:O:s:t:v:")) != -1) {
		switch (inp) {
		case 'b':
 ...
 //We will also want a hash table of this data
 
 
 //Calculate decent hash table size for n's. 
 //Power of 2 hash table is probably most efficient, and we can probably afford to oversize.
 
 int nHashSize = count3;
 int tempCounter = 1;
 while (nHashSize > 0) {
  nHashSize = nHashSize >> 1;
  tempCounter++;  
 }
 nHashSize = 1 << tempCounter;

 cout << "Recommended n hash table size " << nHashSize << endl;
 cout << ((nHashSize * 4)/1024)/1024 << "MB" << endl;

 int *nHashes = (int *)malloc(nHashSize * sizeof(int));
 memset(nHashes, 0, nHashSize * sizeof(int));
	...
	if (myfile2.is_open())
	{
		while (getline(myfile2, line))
		{
			...
			if (token.compare("ABCD") == 0) {
				...
				//Hash the initial n-value and its associated k
				int hashNBucket = total % nHashSize;
				//Blank off the low bits and add the k index
				int hashNValue = ((total >> tempCounter) << tempCounter) + count1 - 1 ;
				if (nHashes[hashNBucket] == 0) {
					nHashes[hashNBucket] = hashNValue;
				}
				else {
					//We were a collision into this bucket, so use linear probing
					while (nHashes[hashNBucket] != 0) {
						hashNBucket++;
					}
				nHashes[hashNBucket] = hashNValue;
				}
				...
				//Hash this total and its associated k
				int hashNBucket = total % nHashSize;
				//Blank off the low bits and add the k index
				int hashNValue = ((total >> tempCounter) << tempCounter) + count1 - 1 ;
				if (nHashes[hashNBucket] == 0) {
	...
	//We are now looking at using 8 masks of roughly equal size. This will help performance while also keeping memory usage lower than using masks consisting of sequential primes
	
    unsigned long long *mask = allocate_and_initialize(19*13*67*11);
    unsigned long long *mask1 = allocate_and_initialize(103*23*17*5);
    unsigned long long *mask2 = allocate_and_initialize(107*97*3*7);
    unsigned long long *mask3 = allocate_and_initialize(43*41*101);
    unsigned long long *mask4 = allocate_and_initialize(83*73*29);
    unsigned long long *mask5 = allocate_and_initialize(61*47*59);
    unsigned long long *mask6 = allocate_and_initialize(31*71*79);
    unsigned long long *mask7 = allocate_and_initialize(37*89*53);

 
 
 
 const unsigned long long bitSetMask = 0x8000000000000000;
   
  
 //Lets try and generalise the use of masks
 int numOfMasks = 8;
 
 int masks0[4] = {19,13,67,11};
 int masks1[4] = {103,23,17,5};
 int masks2[4] = {107,97,3,7};
 int masks3[3] = {43,41,101};
 int masks4[3] = {83,73,29};
 int masks5[3] = {61,47,59};
 int masks6[3] = {31,71,79};
 int masks7[3] = {37,89,53};

 
 int sizes[numOfMasks] = {4,4,4,3,3,3,3,3};
 
 int* (allMasks)[numOfMasks] = {masks0,masks1,masks2,masks3,masks4,masks5,masks6,masks7};
 
 unsigned long long* (maskNew)[numOfMasks] = {mask,mask1,mask2,mask3,mask4,mask5,mask6,mask7}; 
 
 //cout << allMasks[0][4] << endl;

 for (int j = 0; j < numOfMasks; j++) {
  int numMaskElements = sizes[j];
  
  //Calculate the limit value;
  int limit = 64;
  for (int z = 0; z<numMaskElements; z++) {
   limit *= allMasks[j][z];
  }
  
  for (int i = 0; i < numMaskElements; i++) {
   for (int k = 0; k < limit; k += allMasks[j][i]) {
    int intbool = k / 64; // Find the right int
    int intshift = k & 63; // Work out the shift
    maskNew[j][intbool] |= (bitSetMask >> intshift);
   }
  }
 ...
 cudaStatus = cudaMalloc((void**)&dev_a, 2 * arraySize * sizeof(int));
	...
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc f failed!");
		//goto Error;
	}

 //This breaks horribly if you put the whole multiplication on a single line
 unsigned long long dev_gSize = arraySize;
 dev_gSize = dev_gSize * hashTableSize * hashScaling;
 dev_gSize = dev_gSize * sizeof(int);
	cudaStatus = cudaMalloc((void**)&dev_g, dev_gSize);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc g failed!");
		//goto Error;
	}

	cudaStatus = cudaMalloc((void**)&dev_h, sizeof(int));
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc h failed!");
		//goto Error;
	}
 ...
 unsigned long long dev_jSize = arraySize;
 dev_jSize = dev_jSize * hashTableSize * hashScaling;
 dev_jSize = dev_jSize / 32;
 dev_jSize = dev_jSize * sizeof(unsigned int);
 //unsigned long long dev_jSize = ((arraySize * hashTableSize * hashScaling) / 32) * sizeof(unsigned int);
 //cout << "dev_jSize= " << dev_jSize << endl;
	cudaStatus = cudaMalloc((void**)&dev_j, dev_jSize);
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc j failed!");
		//goto Error;
	}
	...
	if (cudaStatus != cudaSuccess) {
		fprintf(stderr, "cudaMalloc n failed!");
		//goto Error;
	}
	...
	generateGPUPrimes(KernelP, low, smallP, testArraySize, primeCount, arraySize, mark1, maskNew, numOfMasks);
	cout << endl;
 ...
 int factorCount = 0;
  ...
  //unsigned long long gSize = arraySize * hashTableSize * hashScaling * sizeof(int);
		cudaMemsetAsync(dev_g, 0, dev_gSize, stream0);
		cudaMemsetAsync(dev_j, 0, dev_jSize, stream0);
  ...
  if (cudaStatus != cudaSuccess) {
    fprintf(stderr, "cudaAsync's have failed!");
  }
  ...
  for (int i=0; i<arraySize; i++) {
    if ((NOut[arraySize+i] > (minN-1))  && (NOut[arraySize+i] < (maxN+1))) {    
      if (verify == 1) {
        ...
        cout << "We're testing to see if " << factorCandidate  << " | " << kVal << "*" << baseCPU << "^" << nVal << "-1" << endl;
		
		int mod = nVal % minQ;
		int m = ((nVal - mod) / minQ) - minimum;
		int bit = m / 32;
		int pos = 31 - (m & 31);
		
		//Use matrix to check for this expression
		for (int k=0; k<minSubs; k++) {
			//Use the row offset so we know we are only looking at k values.
			if (matrix[k*rowoffset] == kVal) {
				//We've found the right k, now find the right remainder
				//cout << "Found k " << matrix[k*rowoffset] << " " << kVal << endl;
				int j=0;
				while (matrix[((k+j)*rowoffset)+1] != mod) {
					j++;
				}
				//cout << "Found rem " << matrix[((k+j)*rowoffset)+1] << " " << mod << endl;
				//Now we have the right row, its ((k+j)*rowoffset)
				//Look for the n-value in here
				//cout << matrix[((k+j)*rowoffset)+bit+2] << endl;
				//cout << pos << endl;
				//int check = 1 << pos;
				//cout << check << endl;
				if ((matrix[((k+j)*rowoffset)+bit+2] & (1 << pos)) != 0) {
					//This is a hit
					
					//This method will be incredibly slow for large values of n. 
			...
			//		}
					...
					//Optimised method to calculate (b^n)%p
					unsigned __int128 val = 1;
					//Check that base is smaller than the factor
					baseCPU = baseCPU%factorCandidate; 
					//Iterate over each bit of n (since n is a 32-bit integer)
					for (int z = 31; z >= 0; --z) {
						val = (val * val) % factorCandidate; // Square the result and take modulus

					// If the i-th bit of n is set, multiply by b and take modulus
						if (nVal & (1U << z)) {
							val = (val * baseCPU) % factorCandidate;
						}
					}
					...
					if (val == 1) {
						...
						//We could set the bit to 0 so that we don't find another factor for the same candidate
					}
				}
				break;
			}
...
//       for (int k=0; k<count3; k++) {    //This is a bad approach because if count3 is huge this takes far too long. Need to use hash tables instead.
//          //Search for a 0
//          if (kns[k] == 0) {
//            if (kns[k+1] == kVal) {
...
//	clock_t begin = clock();
 ...
 //These are the only values that need to be updated in this function when we change the masks
 //These values comes straight from my magic number notebook
 int magicModulos[numOfMasks] = {182039,201365,217959,178063,175711,169153,173879,174529};
 int magicMultipliers[numOfMasks] = {126574,105402,69815,23649,133156,83255,164370,85901};
 unsigned long long offsets[numOfMasks] = {1,1,1,1,1,1,1,1};
 
 
 //This loop works out what the offset values should be for each of the masks and will replace the cases below later.
 for (int i =0; i < numOfMasks; i++) {
  offsets[i] = low % magicModulos[i];
  offsets[i] = (offsets[i] * magicMultipliers[i]) % magicModulos[i];
 }
 
 for (int i = 0; i < ((testArraySize / 64) + 1); i++) {
  mark1[i] = maskNew[0][offsets[0]];
  for (int k =1; k < numOfMasks; k++) {
	 mark1[i] |= maskNew[k][offsets[k]];
	 }
  for (int j = 0; j< numOfMasks; j++) {
   //Increase the offsets and modulo if needed
   offsets[j] = (offsets[j]+1);// % magicModulos[j];
   if (offsets[j] >= magicModulos[j]) {
     offsets[j] = offsets[j] - magicModulos[j];
   }
   }
 }
